...
def main():
    ...
    start_location = tuple(find_position('S', mapped_area))[0]
    other_start_locations = find_position('a', mapped_area)
    other_start_locations.add(start_location)
    target_location = tuple(find_position('E', mapped_area))[0]
    ...
    shortest_path = breadth_first_search(other_start_locations, target_location, mapped_area, walked_area, walked_area_refresh)
...
def find_position(input, mapped_area):
    ...
    for i, location in enumerate(mapped_area):
        for j in enumerate(mapped_area[i]):
            if input in j:
                coordinates.add((i, j[0]))
    return coordinates
